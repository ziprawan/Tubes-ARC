...
const ImageGallery: React.FC<ImageGalleryProps> = ({ mediaUrls, reportTitle }) => {
  ...
  const validmediaUrls =
    mediaUrls?.filter((url) => typeof url === "string" && url.match(/\.(jpeg|jpg|gif|png|webp)$/i)) || [];
  ...
  const goToPrevious = () => {
    setCurrentIndex((prevIndex) => (prevIndex === 0 ? validmediaUrls.length - 1 : prevIndex - 1));
  };

  const goToNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex === validmediaUrls.length - 1 ? 0 : prevIndex + 1));
  };
          ...
          >
            ...
            <div className="flex justify-between items-center mb-3 pb-3 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300">
                {reportTitle ? `Dokumentasi: ${reportTitle}` : "Galeri Gambar"} ({currentIndex + 1} dari{" "}
              ...
              <Image
                ...
                onError={(e) => {
                  ...
                  if (parent) {
                    ...
                    errorDiv.className =
                      "w-full h-[200px] sm:h-[300px] md:h-[400px] flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 rounded";

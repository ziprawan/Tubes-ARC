...
export async function GET() {
  try {
    ...
    const latestReportResult = db.prepare("SELECT title FROM reports ORDER BY created_at DESC LIMIT 1").get() as
      | LatestReport
      | undefined;
    ...
    const mostLikedReportResult = db
      .prepare("SELECT title, likes FROM reports ORDER BY likes DESC, created_at DESC LIMIT 1")

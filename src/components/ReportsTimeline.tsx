...
const getDisasterColor = (type: DisasterType): string => {
  switch (type) {
    case 'earthquake': return 'bg-red-100 text-red-800 border-red-300 dark:bg-red-900 dark:text-red-200 dark:border-red-700';
    case 'volcano': return 'bg-orange-100 text-orange-800 border-orange-300 dark:bg-orange-900 dark:text-orange-200 dark:border-orange-700';
    case 'flood': return 'bg-blue-100 text-blue-800 border-blue-300 dark:bg-blue-900 dark:text-blue-200 dark:border-blue-700';
    case 'tornadoes': return 'bg-purple-100 text-purple-800 border-purple-300 dark:bg-purple-900 dark:text-purple-200 dark:border-purple-700';
...
function ReportCard({ report, onLikeUpdate }: ReportCardProps) {
  ...
  const timeAgo = report.created_at ? formatDistanceToNow(new Date(report.created_at), {
    addSuffix: true,
    locale: id,
  ...
  const borderColorClass = disasterColorClasses.split(' ').find(cls => cls.startsWith('border-'))?.replace('border-', 'border-l-') || 'border-l-gray-300 dark:border-l-gray-600';

  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow border-l-4 ${borderColorClass}`}>
...
export default function ReportsTimeline() {
  ...
  const fetchData = useCallback(async (currentFilter: DisasterType | 'all' = selectedFilter) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const handleLikeUpdateInTimeline = (reportId: string, newLikesCount: number) => {
    const updateReports = (reports: DisasterReport[]) =>
      reports.map(report =>
        report.id === reportId ? { ...report, likes: newLikesCount } : report
      );
    ...
    <div>
      ...
      {isLoading && !allReports.length ? (
        <div className="space-y-6">
          {[...Array(3)].map((_, i) => <ReportCardSkeleton key={`skeleton-${i}`} />)}
        </div>
      ) : error ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            {selectedFilter === 'all' ? "Belum ada laporan yang masuk." : `Belum ada laporan untuk kategori "${getDisasterTypeLabel(selectedFilter as DisasterType)}".`}
          </p>
        </div>
      ) : (
        <div className="space-y-6">
